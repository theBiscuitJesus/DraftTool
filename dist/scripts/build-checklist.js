...
async function fetchSet(query) {
    ...
    while (url) {
        ...
        if (!res.ok)
            throw new Error(`Scryfall ${res.status} for ${url}: ${await res.text()}`);
...
if (mainCards.length !== 276)
    throw new Error(`expected 276 main cards, got ${mainCards.length}`);
if (mar.length !== 60)
    throw new Error(`expected 60 mar cards, got ${mar.length}`);
for (const [page, row, name] of checks) {
    const e = entries.find(x => x.page === page && x.row === row);
    if (e?.name !== name)
        throw new Error(`spot check failed: ${page}/${row} = ${e?.name}, expected ${name}`);
}
