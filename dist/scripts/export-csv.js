...
const USAGE = 'usage: export-csv [DIR | page.json ...] [--out FILE] [--pool N|none] [--checklist FILE]\n' +
...
const tag = (last3, lastName) => last3 || lastName.slice(0, 3);
export function run(args) {
    ...
    let expectedPool = null;
    const poolIdx = args.indexOf('--pool');
    if (poolIdx >= 0) {
        const v = args[poolIdx + 1];
        if (v === undefined || v.startsWith('--'))
            return { exitCode: 1, stdout: USAGE };
        if (/^(none|off|skip)$/i.test(v))
            expectedPool = null;
        else if (/^\d+$/.test(v))
            expectedPool = Number(v);
        else
            return { exitCode: 1, stdout: `ERROR: --pool expects a number or "none", got "${v}"\n` };
    }
    ...
    for (const { page, file } of loaded.filter(l => l.page.page === 'main')) {
        ...
        if (!g) {
            g = { key, table, lastName, last3, pages: [] };
            groups.push(g);
        }
        g.pages.push(page);
    }
    for (const { page, file } of loaded.filter(l => l.page.page === 'mar')) {
        ...
        const matches = groups.filter(g => {
            if (!hasHeader)
                return false;
            if (table && g.table && table !== g.table)
                return false;
            const grpTag = tag(g.last3, g.lastName);
            if (marTag && grpTag && marTag !== grpTag)
                return false;
            return true;
        });
        if (matches.length === 1)
            matches[0].pages.push(page);
        else if (!hasHeader && groups.length === 1)
    ...
    for (const g of groups) {
        ...
        for (const i of issues)
            summary.push(`      ${i.severity.toUpperCase()}: ${i.message}`);
    }
