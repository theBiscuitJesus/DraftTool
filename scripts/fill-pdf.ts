...
import {
  extractMainPositions,
  extractMarPositions,
  loadPdfBytes,
  type BasicKey,
  type BoxPos,
  type PagePositions,
} from '../lib/pdf-positions.js'
...
function digitBaselineY(centerY: number): number {
...
function drawDigit(page: PDFPage, font: PDFFont, value: number, x: number, centerY: number): void {
...
function drawHighlight(page: PDFPage, box: BoxPos, color: ReturnType<typeof rgb>): void {
...
function playedSum(page: RecognizedPage): number {
  const rowSum = page.rows.reduce((s, r) => s + (r.played ?? 0), 0)
  const basicsSum = Object.values(page.basics ?? {}).reduce((s: number, v) => s + (v ?? 0), 0)
  return rowSum + basicsSum
}

function poolSum(page: RecognizedPage): number {
  const rowSum = page.rows.reduce((s, r) => s + (r.total ?? 0), 0)
  const basicsTotalSum = Object.values(page.basicsTotal ?? {}).reduce((s: number, v) => s + (v ?? 0), 0)
...
// from the pool.
function drawTallyBox(page: PDFPage, font: PDFFont, played: number, pool: number): void {
...
async function positionsFor(page: RecognizedPage, pdfBytes: Uint8Array): Promise<PagePositions> {
...
export async function fillPdf(
  ...
  tally?: { played: number; pool: number },
): Promise<Uint8Array> {
  ...
  if (!targetPage) throw new Error(`source PDF has no page for "${recognized.page}"`)
  ...
  for (const r of recognized.rows) {
    ...
    if (highlight) drawHighlight(targetPage, box, highlight)
    if (r.played !== null && r.played !== undefined) drawDigit(targetPage, font, r.played, box.playedX, box.centerY)
    if (r.total !== null && r.total !== undefined) drawDigit(targetPage, font, r.total, box.totalX, box.centerY)
  }

  if (recognized.basics) {
    for (const [key, box] of positions.basics) {
      const value = recognized.basics[key as BasicKey]
      if (value && value > 0) {
  ...
  if (positions.header && recognized.player) {
    ...
    if (firstName) targetPage.drawText(firstName, { x: h.firstNameX, y: nameY, size: 8, font, color: DARK_BLUE })
    if (lastName) targetPage.drawText(lastName, { x: h.lastNameX, y: nameY, size: 8, font, color: DARK_BLUE })
    if (table) targetPage.drawText(table, { x: h.tableX, y: h.tableY, size: 11, font, color: DARK_BLUE })
  }
...
async function run(args: string[]): Promise<{ exitCode: number; stdout: string }> {
  ...
  if (outIdx >= 0) {
    ...
    if (value === undefined || value.startsWith('--')) return { exitCode: 1, stdout: USAGE }
    outDir = value
  }
  const files = args.filter((a, i) => !a.startsWith('--') && args[i - 1] !== '--out')
  if (files.length === 0) return { exitCode: 1, stdout: USAGE }
  ...
  const pages: Array<{ file: string; recognized: RecognizedPage }> = []
