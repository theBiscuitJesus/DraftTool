...
export type BoxPos = { playedX: number; totalX: number; centerY: number }
export type BasicKey = 'plains' | 'island' | 'swamp' | 'mountain' | 'forest'

export type HeaderPositions = {
  firstNameX: number
  lastNameX: number
  lineY: number
  tableX: number
...
export type PagePositions = {
  rows: Map<number, BoxPos>
  basics: Map<BasicKey, BoxPos>
...
function nearestLaneNameX(numX: number, lanes: { numXRef: number; nameX: number }[]): number {
  let best = lanes[0]
...
function boxFromAnchor(nameX: number, anchorY: number): BoxPos {
  return {
...
async function getPageTextItems(pdfBytes: Uint8Array, pageIndex: number): Promise<TextItem[]> {
  const pdfjsLib = await import('pdfjs-dist/legacy/build/pdf.mjs')
  const doc = await pdfjsLib.getDocument({ data: pdfBytes }).promise
  const page = await doc.getPage(pageIndex + 1)
  const content = await page.getTextContent()
  return content.items as unknown as TextItem[]
}

const BASIC_LABELS: Record<string, BasicKey> = {
  Plains: 'plains',
  Island: 'island',
  Swamp: 'swamp',
  Mountain: 'mountain',
...
export async function extractMainPositions(pdfBytes: Uint8Array): Promise<PagePositions> {
  const items = await getPageTextItems(pdfBytes, 0)
  const rows = new Map<number, BoxPos>()
  const basics = new Map<BasicKey, BoxPos>()
  let firstNameX: number | undefined
  let lastNameX: number | undefined
  ...
  for (const it of items) {
    ...
    if (it.str.startsWith('Player') && it.str.includes('First Name')) firstNameX = x
    if (it.str.startsWith('Player') && it.str.includes('Last Name')) lastNameX = x
  }

  const header: HeaderPositions | undefined =
    firstNameX !== undefined && lastNameX !== undefined
...
export async function extractMarPositions(pdfBytes: Uint8Array): Promise<PagePositions> {
...
export function loadPdfBytes(path = 'data/checklist-sheet.pdf'): Uint8Array {
